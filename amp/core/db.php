...
$mySQL = new db($config->host, $config->{"user name"}, $config->password, $config->{"db name"});
...
class db extends mysqli{
	...
	public function getPost($id){
		$row = $this->getRow("
		SELECT ID,PageID,header,subheader,preview,ads,language,created,category,subtemplate,gb_blogfeed.tid as tid,Name,content
		FROM gb_blogfeed
		...
		WHERE ID = {int} AND created<{int} ORDER BY (POW(2,language-1) & {int}) LIMIT 1", $id, time(), LANG_MASK);
		return $row;
	}
	...
	public function login(){
		...
		$row = $this->getRow("SELECT * FROM `cl_sessions` CROSS JOIN `cl_users` USING(`UserID`) WHERE `finger` LIKE {str} LIMIT 1", $_COOKIE['finger']);
		if(empty($row)){
			$rows = $this->get("SELECT * FROM `cl_users` LEFT JOIN `cl_sessions` USING(`UserID`)");
		...
		if(!in_array($_SERVER['REMOTE_ADDR'], $vars['IPs'])){
			...
			$d = array_slice($d, 1);
			sendmail($row['email'], $message, $mode="plain", $theme="Sign in with a new IP", $d[0]." <login-protect@".implode(".", $d).">");
		}
		$set = $this->parse("{set}",[ "finger"=>$finfer, "variables"=>$row['variables'] ]);
		$this->inquiry("INSERT INTO `cl_sessions` SET `UserID`={int}, {prp} ON DUPLICATE KEY UPDATE {prp}",USER_ID, $set, $set);
