...
<channel>
	<title>Lifter | Помогает расти</title>
	<link>https://lifter.com.ua/</link>
	...
	<?foreach($feed as $item):
		$canonical = PROTOCOL."://".$_SERVER['HTTP_HOST']."/".translite($item['header'])."-".$item['PageID'];
		?>
		<item>
			...
			<![CDATA[
				...
				<body>
					<article>
						...
						<header>
							<figure class='op-ad'>
								<iframe width='300' height='250' style='border:0; margin:0;' src='https://www.facebook.com/adnw_request?placement=274396629918601_274396663251931&adtype=banner300x250'></iframe>
							</figure>
							<figure>
								<?if(empty($item['video'])):$uri = parse_url($item['video'])?>
								<img src="<?=$item['preview']?>">
								<?else:?>
								<video><source src="<?=$item['video']?>" type="<?=mime_content_type("../".array_shift(explode(".",$uri['host']))."".$uri['path'])?>"/></video>
								<?endif?>
							</figure>
							<h1><?=$item['header']?></h1>
	...
	foreach($feed as $item):
		$canonical = PROTOCOL."://".$_SERVER['HTTP_HOST']."/".translite($item['header'])."-".$item['ID'];
		?>
		<item>
			<title><?=$item['header']?></title>
			<link><?=(PROTOCOL."://".$_SERVER['HTTP_HOST']."/".translite($item['header'])."-".$item['ID'])?></link>
			<guid><?=(PROTOCOL."://".$_SERVER['HTTP_HOST']."/".$item['language']."/".$item['ID'])?></guid>
			<pubDate><?=date("c", $item['created'])?></pubDate>
			<author><?=$item['Name']?></author>
			<description><?=$item['subheader']?></description>
			<content:encoded>
				...
				<body>
					<article>
						...
						<header>
							...
							<figure><?if($item['cover']=="video"):$uri = parse_url($item['video'])?><video><source src="<?=$item['video']?>" type="<?=mime_content_type("../".array_shift(explode(".",$uri['host']))."".$uri['path'])?>"/></video><?else:?><img src="<?=$item['preview']?>"><?endif?></figure>
							<h1><?=$item['header']?></h1>
							<h2><?=$item['subheader']?></h2>
							<time class="op-published" dateTime="<?=date("c", $item['created'])?>"><?=date("d F, H:i",$item['created'])?></time>
							<time class="op-modified" dateTime="<?=date("c", $item['modified'])?>"><?=date("d F, H:i", $item['modified'])?></time>
