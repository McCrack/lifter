...
class Data{
	...
	public function __construct(){
		...
		if(PAGE_ID){
			...
			PAGE_ID, LANG_MASK);
		}else $GLOBALS['mySQL']->status = ["affected_rows"=>0];
		...
		if($GLOBALS['mySQL']->status['affected_rows']>0){
			...
			if(OLD_POST){
				$this->data['name'] = translite($this->data['header'])."-".$this->data['ID'];
			}else $this->data['name'] = translite($this->data['header'])."-".$this->data['PageID'];
			$this->title = implode(" - ",[
				$this->data['header'],
				$this->SiteName
			]);
	...
	private function post(){
		if($GLOBALS['mySQL']->status['affected_rows']>0){
			if(OLD_POST){
				return $GLOBALS['mySQL']->getRow("SELECT * FROM gb_blogfeed CROSS JOIN gb_pages USING(PageID) CROSS JOIN gb_blogcontent USING(PageID) WHERE ID={int} LIMIT 1", PAGE_ID);
			}else return $GLOBALS['mySQL']->getRow("SELECT * FROM gb_pages CROSS JOIN gb_blogfeed USING(PageID) CROSS JOIN gb_blogcontent USING(PageID) WHERE PageID={int} LIMIT 1", PAGE_ID);
		}else{
			header('HTTP/1.0 404 Not Found');
			return $GLOBALS['mySQL']->getMaterial("404", ["*"]);
		}
