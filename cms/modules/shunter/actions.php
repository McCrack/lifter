...
switch(ARG_2){
	...
	case "take-to-work":
		...
		$log .= " -> Take to work;\n";
		$mySQL->inquiry("UPDATE gb_task_timing SET towork={int},CommunityID={int},log=CONCAT(log,{str}) WHERE TaskID={int} LIMIT 1", time(),$p['performer'],ARG_4,$log,ARG_3);
		$mySQL->inquiry("UPDATE gb_task_shunter SET status='in work' WHERE TaskID={int} LIMIT 1", ARG_3)['affected_rows'];
