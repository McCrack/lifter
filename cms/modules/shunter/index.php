			...
			<section>
				<div class="tabs">
					...
					<form id="task" class="tab t light-btn-bg" autocomplete="off">
						...
						<?if(empty($task['PageID'])):?>
						<button title="Create Post" type="submit" class="create-post">&#xe926;</button>
						<?else:?>
						<a href="/blogger/ru/1/<?=$task['PageID']?>">Open Post</a>
						<?endif?>
						...
						(function(form){
							...
							form.onsubmit=function(event){
								...
								XHR.push({
									addressee:"/blogger/actions/create-post",
									onsuccess:function(response){
										try{
											response = JSON.parse(response);
											setTimeout(function(){
												location.pathname = ["blogger",response['language'],1,response['PageID']].join("/");
											},2000);
											XHR.push({
												addressee:"/shunter/actions/take-to-work/<?=$task['TaskID']?>/<?=COMMUNITY_ID?>",
											});
