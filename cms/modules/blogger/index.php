...
if(defined("ARG_3")){
	if(is_numeric(ARG_3)){
		$post = $mySQL->getRow("
		SELECT
		PageID,ID,SetID,language,published,approved,header,subheader,preview,category,created,video,Ads,
		...
		$card = $mySQL->getRow("
		SELECT * FROM gb_task_shunter
		CROSS JOIN gb_task_timing USING(TaskID)
		LEFT JOIN gb_community USING(CommunityID)
		WHERE PageID = {int} LIMIT 1", PAGE_ID);
		...
		<div id="wrapper">
						...
						<div id="feed">
							...
							<a class="snippet" href="/blogger/<?=$row['language']?>/<?=$page?>/<?=$row['PageID']?>">
								<div class="preview">
									...
									<?if((INT)$row['approved']):?><img src="/themes/default/images/approved.png"><?endif?>
								</div>
								<div class="header"><?=$row['header']?> :<?=$row['approved']?></div>
								<div class="options">
			...
			<section>
				<div class="tabs">
					...
					<form id="metadata" class="tab body-bg light-txt" autocomplete="off">
						...
						<div class="black-bg" align="right">
							...
							<button name="approved" id="approved">Approved</button>
						</div>
						<div id="cover">
						...
						(function(form){
							...
							form.approved.onclick=function(event){
								event.preventDefault();
								XHR.push({
									addressee:"/blogger/actions/approved/<?=$post['PageID']?>"
								});
							}
				...
				<form id="rightbar-tabs" class="v-bar r v-bar-bg" data-default="right-default" autocomplete="off">
					...
					<div class="toolbar">
						<?if(!empty($card)):?>
						<input id="show-task" type="checkbox" hidden>
						<label for="show-task" title="task card" class="tool" data-translate="title">&#xf066;</label>
						<div class="card" data-id="<?=$card['TaskID']?>">
							<div class="snippet">
								<div class="preview"><img src="<?=$card['image']?>" alt="&#xe94a;"></div>
								<div class="header"><span><?=$card['header']?></span></div>
								<div class="task"><?=$card['header']?></div>
								<hr size="1" color="#555">
								<div class="right gold-txt"><?=$card['Name']?></div>
								<div class="select">
									<select name="status" class="active-txt">
									<?foreach(["new","in work","done","waste","deleted"] as $status):?>
										<?if($card['status']==$status):?>
										<option selected value="<?=$status?>"><?=$status?></option>
										<?else:?><option value="<?=$status?>"><?=$status?></option><?endif?>
									<?endforeach?>
									</select>
									<script>
									(function(select){
										select.onchange=function(event){
											XHR.push({
												addressee:"/shunter/actions/change-status/<?=$card['TaskID']?>",
												body:JSON.encode({
