<?switch(ARG_2){
	...
	break;
	case "approved":
		print $mySQL->inquiry("UPDATE gb_blogfeed SET approved=1 WHERE PageID={int} LIMIT 1", ARG_3)['affected_rows'];
	break;
	case "remove":
		print $mySQL->inquiry("DELETE FROM gb_pages WHERE PageID={int} LIMIT 1", ARG_3)['affected_rows'];
	break;
	case "drop-amp":
