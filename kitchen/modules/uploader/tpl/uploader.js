		...
		switch(type){
			default: if(SHOWFILES){
				if(type!="image"){
					...
					if(type!="video"){
						if(ONLY_VIDEOS) continue;
						if(type!="text") type = "application";
					}
				}else if(ONLY_VIDEOS) continue;
				root.appendChild( doc.create("span", key, {"class":"file", "data-type":type, "data-mime":obj[key]['type'], "data-path":obj[key]['path']}) );
			} break;
			case "folder":
...
function showCurrentFolder(obj){
	let types = ["html","text","zip","pdf","audio"];
	for(var key in obj){
		if(!obj.hasOwnProperty(key)) continue;
		if(obj[key]['type'] === "folder"){
			...
			if(type[0] === "image"){
				...
				var preview = "/images/mime/video.png";
			}else if(ONLY_VIDEOS){
				continue;
...
function Open(event){
	...
	//alert(obj.dataset.type);
	let mode = {
		"image":"file",
		"video":"file",
		"text":"file",
		"application":"file"
	}[obj.dataset.type] || obj.dataset.type;
	if(mode==="folder" || mode==="openfolder"){
		reloadExplorer(obj.dataset.path);
	}else if(mode==="file")	openFile(obj.dataset.path, obj.dataset.mime);
}
