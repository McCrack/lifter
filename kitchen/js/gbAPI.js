		...
		box.onopen = function(){
			...
			for(var i=files.length; i--;){
				for(var j=0; j<files[i].size; j+=BLOCK_SIZE){
					seek = j + BLOCK_SIZE;
					var filename = [];
					files[i].name.split(/\./).forEach(function(substr){ filename.push(substr.translite()); });
					XHR.push({
						"seek":seek,
						"not_last":i,
						"size":files[i].size,
						"Content-Type":"application/octet-stream",
						"addressee":addressee+"&file="+filename.join(".")+"&seek="+j,
...
var Box = function(params, source, protect){
			...
			"onsuccess":function(response){
				...
				if(box.onsubmit){
					box.window.onsubmit = function(event){
						...
						box.window.querySelectorAll("input, textarea").forEach(function(item){
							var pattern = item.getAttribute("pattern");
							if(pattern && !item.value.isFormat(pattern)){
								return false;
							}
						});
						box.onsubmit(this)
	...
	this.align = function(){
		box.body.style.maxHeight=(doc.height-150)+"px";
