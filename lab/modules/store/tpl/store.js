...
function createModel(){
	...
	var box = modalBox('{}', "store/createbox/"+(path[2] || 0), function(form){
		XHR.push({
			...
			"onsuccess":function(response){
				isNaN(response) ? alertBox(response) : location.pathname = "/store/showcase/"+response;
			}
		});
	}, true);
}
...
function saveOptions(){
	...
	for(var i=0; i<cells.length; i+=2){
		...
		if(key){
			properties[key] = cells[i+1].textContent.trim();
		}
	}
	XHR.push({
...
function addFilterSet(){
	promptBox("Please enter  name of filter set", function(value){
		...
		doc.querySelector("#filterset").appendChild(set);
	});
}
...
function addFilterValue(row){
	row.insertAfter( doc.create("tr", "<th title='Add value' onclick='addFilterValue(this.parentNode)' bgcolor='white'><span class='tool'>&#xe908;</span><td onfocus='focusCell(this)' contenteditable='true'></td></th><th bgcolor='white'><span title='Drop row' class='tool red' onclick='deleteRow(this)'>&#xe907;</span></th>", {"bgcolor":"#DEF"}) );
}