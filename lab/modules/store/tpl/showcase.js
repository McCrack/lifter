		...
		XHR.push({
			...
			"onsuccess":function(response){
				if(isNaN(response)){
					if(isNaN(response)){
						alertBox(response);
					}else location.pathname = "/store/showcase/"+response;
...
function saveItem(){
	...
	var params = {
		...
		"filters":(function(){
			var section = {"0":0};
			var inp = doc.querySelectorAll("#filters>fieldset>label>input");
			for(var i=inp.length; i--;){
				if(inp[i].checked){
					section[(inp[i].value / 32)>>0] |= Math.pow(2, (inp[i].value % 32)-1);
				}
			}
			return section;
		})()
