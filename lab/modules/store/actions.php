...
$brancher->auth(array("")) or die("Access denied!");
...
switch(SUBPAGE){
	case "reload":
		...
		$category = $mySQL->single_row("SELECT `PageID` FROM `gb_sitemap` WHERE `title` LIKE '".$p['category']."' LIMIT 1");
		...
		foreach($models as $model){
			$stikers .= "
			<a class='sticker' href='/showcase/".$model['PageID']."'>
	...
	break;
	case "create":
		$PageID = $mySQL->insert("gb_pages", array("created"=>time()));
		$mySQL->insert("gb_models", ["PageID"=>$PageID, "CategoryID"=>PARAMETER, "articul"=>(PARAMETER."-".$PageID), "tid"=>2]);
		print($PageID);
	break;
	case "add":
...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	...
	case "save-description":
		...
		$saved = $mySQL->update("gb_models",  array("description"=>$mySQL->escape_string($data)), "`PageID`=".PARAMETER);
		print($saved);
	break;
	case "save-stock":
		$p = JSON::load('php://input');
	...
	case "save-options":
		...
		$category = $mySQL->single_row("SELECT `PageID` FROM `gb_sitemap` WHERE `title` LIKE '".PARAMETER."' LIMIT 1");
	...
	case "save-filterset":
		...
		$category = $mySQL->single_row("SELECT `PageID` FROM `gb_sitemap` WHERE `title` LIKE '".PARAMETER."' LIMIT 1");
		$mySQL->query("UPDATE `gb_static` SET `filterset`='".JSON::stringify($p)."' WHERE `PageID`=".$category['PageID']." LIMIT 1");
