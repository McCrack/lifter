...
switch(SUBPAGE){
	...
	case "add-page":
		...
		$PageID = $mySQL->insert("gb_pages", array("created"=>time(), "type"=>$p['type']));
		if(empty($PageID)){
			print("unknow error");
		}else{
			...
			if(empty($page)){
				...
				$mySQL->single_row("INSERT INTO `gb_static` (`PageID`,`changed`) VALUES (".$PageID.", ".time().")");
				print($PageID);
			}
		}
	break;
	...
	case "save-keywords":
		$data = file_get_contents('php://input');
		$saved = $mySQL->update("gb_static",  ["keywords"=>$mySQL->escape_string($data)], "`PageID`=".PARAMETER);
		print $saved;
	break;
	case "save-header":
		$data = file_get_contents('php://input');
	...
	case "save-metadata":
		...
		if(empty($p['id'])){
			...
			if(empty($material)){
				$p['id'] = $mySQL->insert("gb_pages", ["created"=>$time, "type"=>$p['type']]);
			...
			$mySQL->query("UPDATE `gb_pages` SET `type`='".$p['type']."' WHERE `PageID`=".$p['id']." LIMIT 1");
	...
	case "save-content":
		...
		$mySQL->update("gb_static",  array("content"=>$mySQL->escape_string($data), "changed"=>time()), "`PageID`=".PARAMETER);
