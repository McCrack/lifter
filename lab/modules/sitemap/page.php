...
?>
					...
					<div id="explorer">
						<div class="root">
							<a class="tree-root-item" href="/sitemap/<? print $language; ?>">Root</a>
		...
		<div id="rightbar">
			<aside class="tabbar right" onclick="openTab(event.target, 'rightbar')">
				<div class="toolbar">
					...
					<span class="tool" data-translate="title" title="properties" data-tab="properties">&#xf05e;</span>
			...
			<form id="meta" class="tab" autocomplete="off">
				<div class="environment">
					<div class="caption">
						<small>ID:</small> <input name="pageID" value="<?php print($post['PageID']); ?>" readonly size="3" placeholder="0">
						<!-- language -->
						<input name="language" readonly value="<?php print($language); ?>" size="3">
						<!-- Parent -->
						<div class="right"><small>Parent:</small> <input name="prt" value="<?php print($post['parent']); ?>" size="12"></div>
					</div>
			<!-- Type -->
			...
			<!-- URL Name -->					
					<fieldset><legend data-translate="textContent">page name</legend>
						...
						<input name="url" value="<?php print($name); ?>" placeholder="..." required style="width:calc(100% - 32px)">
					</fieldset>
			<!-- Module -->
...
?>
			...
			<!-- Header -->
					<fieldset><legend data-translate="textContent">header</legend>
						<input name="header" value="<?php print($post['header']); ?>" placeholder="..." required>
					</fieldset>
			<!-- Template -->
...
<?php
	...
	foreach(scandir("../".BASE_FOLDER."/themes/".$cng['general']['theme']['value']) as $file){
		$file = explode(".", $file);
		if(end($file) === "html"){
...
?>
			...
			<!-- Subheader -->
					<fieldset><legend data-translate="textContent">subheader</legend>
						<input name="subheader" value="<?php print($post['subheader']); ?>" placeholder="..." required>
					</fieldset>
			<!-- Context -->
					<fieldset style="width:99%;"><legend data-translate="textContent">context</legend>
						<input name="context" value="<?php print($post['context']); ?>" placeholder="..." required>
					</fieldset>
					<img id="preview" src="<?php print(empty($post['preview']) ? "/images/NIA.jpg" : $post['preview']); ?>" height="284">
				</div>
				<div class="caption">Description</div>
				<textarea name="desc" id="description" placeholder="..."><?php print($post['description']); ?></textarea>
				<div class="caption">Keywords</div>
				<textarea name="keywords" id="keywords" placeholder="..."><?php print($post['keywords']); ?></textarea>
			</form>
			...
			<div id="properties" class="tab">
				<div class="caption">
					<span data-translate="textContent">Footer Blocks</span>
					<div class="toolbar right">
		...
		if(empty($properties)){?>
			<tr>
				<th title="Add value" onclick="addBlockRow(this.parentNode)" bgcolor="white"><span class="tool">&#xe908;</span></th>
				<td onfocus="focusCell(this)" contenteditable="true"></td>
				<th bgcolor="white" title="Delete row" onclick="deleteRow(this)"><span class='tool red'>&#xe907;</span></th>
			</tr>
		<?}else foreach($properties as $val):?>
			<tr>
				<th title="Add value" onclick="addBlockRow(this.parentNode)" bgcolor="white"><span class="tool">&#xe908;</span></th>
...
<?php	
	$options .= "<option value='".$post['footer']."' selected>".$post['footer']."</option>";
	foreach(scandir("../".BASE_FOLDER."/themes/".$cng['general']['theme']['value']."/footers") as $file){
		$file = explode(".", $file);
		if(end($file) === "html"){
			if($file[0] === $post['footer']) continue;
			$options .= "<option value='".$file[0]."'>".$file[0]."</option>";
		}
	}
	print($options);
?>
		...
		<script>
			...
			(function(obj){
				...
				frame.onload = function(){
					...
					//editor.addCSSFile("<?php print(BASE_DOMAIN."/tpls/".$cng['general']['theme']."/content.css"); ?>");
