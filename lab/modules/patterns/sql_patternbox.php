...
boxList[handle].getSQLPattern=function(obj){
...
boxList[handle].createPatternsFolder=function(){
	promptBox("new folder name", function(folderName){
		XHR.push({
			...
			"addressee":"/patterns/actions/create-folder/sql/"+folderName.trim().toLowerCase(),
			"onsuccess":function(response){
				var box = boxList[boxList.onFocus];
...
boxList[handle].removePatterns=function(){
	...
	<div class="box-title">
		...
		<span style="color:#5AD;">MySQL</span>Pattern
	</div>
	<div class="box-body" style="
		...
		<div class="panel" align="left">
			...
			<input name="path" value="patterns/sql" placeholder="path" data-translate="placeholder" required readonly style="
				width:454px;
				padding:4px 10px;
				border-radius:3px;
		...
		<div class="leftbar" onclick="boxList[handle].getSQLPattern(event.target)" style="height:calc(100% - 36px);overflow-y:scroll;padding:15px;box-sizing:border-box;">
<?php
...
function patterns_tree($path="patterns/sql"){
	$items = $dirs = "";
